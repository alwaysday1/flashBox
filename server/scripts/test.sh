...
mb::test::login()
{
  ${CCURL} "${Header}" http://${INSECURE_SERVER}/login \
    -d'{"username":"root","password":"miniblog1234"}' | grep -Eo 'token[" :]+\K[^"]+'
}
...
mb::test::post()
{
  ...
  tokenStr=`${CCURL} "${Header}" http://${INSECURE_SERVER}/login -d'{"username":"posttest","password":"miniblog1234"}' | grep -Eo 'token[" :]+\K[^"]+'`
  token="-HAuthorization: Bearer ${tokenStr}"

  # 3. 创建一个博客
  postID=`${CCURL} "${Header}" "${token}" http://${INSECURE_SERVER}/v1/posts -d'{"title":"installation","content":"installation."}' | grep -Eo 'post-[a-z0-9]+'`
