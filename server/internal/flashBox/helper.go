...
const (
	// recommendedHomeDir 定义放置 flashBox 服务配置的默认目录.
	recommendedHomeDir = ".flashBox"

	// defaultConfigName 指定了 flashBox 服务的默认配置文件名.
	defaultConfigName = "flashBox.yaml"
)
...
func initConfig() {
	...
	// 读取环境变量的前缀为 FLASHBOX，如果是 flashBox，将自动转变为大写。
	viper.SetEnvPrefix("FLASHBOX")
